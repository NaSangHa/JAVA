...
<body>
	...
	<div class="container">
  	  <div class="row justify-content-end text-center">
	  	<div class="col-2">
	  		<button type="button" id="" class="btn btn-secondary" onclick="javascript:window.location='notice.board?page=1'" >검색 초기화</button><br>
		</div>
	  </div>	  
	</div>
	        ...
	        <tr>
	            <td id="page" class="row justify-content-center col-12 align-items-center">
	                ...
	                <c:choose>
		                ...
		                <c:otherwise>
		                    <!-- <a href="search_notice.board?page=1">[처음으로]</a> -->
		                    <a class="page-link col-1 text-center" href="search_notice.board?page=1&search_method=<%= session.getAttribute("search_method") %>&search_content=<%= session.getAttribute("search_content") %>"><<</a>
		                </c:otherwise>
	                </c:choose>
	                &nbsp;&nbsp;
	                ...
	                <c:otherwise>
	                    ...
	                    <a class="page-link col-1 text-center" href="search_notice.board?page=${page.curPage-1}&search_method=<%= session.getAttribute("search_method") %>&search_content=<%= session.getAttribute("search_content") %>"><</a>
	                </c:otherwise>
	                </c:choose>
	    			&nbsp;&nbsp;
	                ...
	                <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
	                    ...
	                    <c:otherwise>
	                        ...
	                        <a class="page-link active" href="search_notice.board?page=${fEach}&search_method=<%= session.getAttribute("search_method") %>&search_content=<%= session.getAttribute("search_content") %>">${fEach} <span class="sr-only">(current)</span></a>
	                    </c:otherwise>
	                    </c:choose>
	                </c:forEach>
	    			&nbsp;&nbsp;
	                ...
	                <c:otherwise>
	                   <%--  <a href="notice.board?page=${page.totalPage}">[끝으로]</a> --%>
	                    <a class="page-link col-1 text-center" href="search_notice.board?page=${page.totalPage}&search_method=<%= session.getAttribute("search_method") %>&search_content=<%= session.getAttribute("search_content") %>">>></a>
	                </c:otherwise>
	                </c:choose>
	            </td>
	        </tr>	  
